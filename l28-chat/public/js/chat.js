$.fn.Chat = function (params) {
    ...
    let methods = {
        ...
        loadMessages: function () {
            $.ajax({
                url: CHAT_ADDRESS + '/room/' + properties.activeRoomId + '/messages',
                type: 'get',
                success: function (data) {
                    container.find('.msg_card_body').html('');
                    methods.renderMessages(data);
                }
            });
        },
        renderMessages: function (messages)
        {
            let messagesWrapper = container.find('.msg_card_body');

            for (let index in messages) {
                let message = messages[index];
                let html = (message['user_id'] === properties.userId)
                    ? methods.renderOwnMessage(message)
                    : methods.renderOtherMessage(message);

                messagesWrapper.append(html);
                properties.messagesHeight += html.outerHeight(true);
            }
            ...
            return $('<div/>')
                ...
                .append(
                    $('<div/>')
                        ...
                        .append(
                            $('<img/>')
                                .attr('src', 'https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg')
                                .addClass('rounded-circle user_img_msg')
                        )
                )
                .append(
                    $('<div/>')
                        .addClass('msg_cotainer')
                        .append(message['text'])
                        .append(
                            $('<span/>')
                                .addClass('msg_time')
                                .text(methods.formatDateTime(message['created_at']))
                        )
                );
        },
        renderOtherMessage: function (message)
        {
            return $('<div/>')
                .addClass('d-flex justify-content-end mb-4')
                .append(
                    $('<div/>')
        ...
        formatDateTime: function (dateTime, useShortMonths = true)
        {
            ...
            let monthName = useShortMonths ? monthsShort[time.getMonth()] : months[time.getMonth()];
    ...
    let events = {
        init: function () {
            ...
            container.find('.chat-room').on('click', events.activateRoom);

            properties.webSocket.onmessage = events.acceptMessage;

            methods.setupDefaultRoom();
        },
        ...
        sendMessage: function (e) {
            ...
            let message = JSON.stringify({
                ...
                data: {
                    ...
                    roomId: properties.activeRoomId,
        ...
        activateRoom: function (e) {
            ...
            let element = $(this);
            properties.activeRoomId = element.data('roomId');
            element.siblings('.chat-room').removeClass('active');
            ...
            let intervalId = setInterval(() => {
                if (properties.webSocket.readyState === 1) {
                    properties.webSocket.send(message);
        ...
        acceptMessage: function (e) {
            ...
            methods.renderMessages([message])
